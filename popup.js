...
function renderCategories () {
  ...
  // "All" pill
  el.appendChild(makePill('all', 'Todos', null,
    state.snippets.length, activeCat === 'all'));
...
function renderSnippets () {
  ...
  if (!items.length) {
    ...
    if (searchQuery) {
      ...
      empty.querySelector('.empty-icon')?.replaceWith(Object.assign(document.createElement('div'),{className:'empty-icon',textContent:'📋'}));
  ...
  items.forEach(s => list.appendChild(makeCard(s)));
}
...
function makeCard (snippet) {
  ...
  card.innerHTML = `
    <style>.snippet-card[data-sid="${snippet.id}"]::before{background:${col}}</style>
    <div class="card-top">
      <span class="card-title">${esc(snippet.title)}</span>
    ...
    <span class="card-badge" style="background:${col}12;color:${col};border:1px solid ${col}30">
      ${esc(cat?.name || 'General')}
    </span>
    <p class="card-preview">${esc(snippet.text)}</p>
    <div class="card-footer">
      <span class="copy-hint">
        ...
        <span class="copy-label">Click para copiar</span>
      </span>
    </div>
  `;

  card.dataset.sid = snippet.id;

  // Click card → copy
  card.addEventListener('click', e => {
    if (e.target.closest('.btn-icon')) return;
    copyText(snippet.text, card);
  });
...
async function copyText (text, card) {
  ...
  setTimeout(() => {
    ...
    if (label) label.textContent = 'Click para copiar';
  }, 1400);
...
function selectColor (c) {
  ...
  document.querySelectorAll('.color-dot').forEach(d =>
    d.classList.toggle('selected', d.style.background === c ||
      ...
      rgbToHex(d.style.backgroundColor) === c)
  );
}
